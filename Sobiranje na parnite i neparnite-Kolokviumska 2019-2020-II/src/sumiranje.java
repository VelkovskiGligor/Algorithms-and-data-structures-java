...
public class sumiranje {
	public static void zberi (SLL<Integer> lista){
		SLLNode<Integer> dvizi=lista.getFirst();
		SLLNode<Integer> parni=null;
		SLLNode<Integer> neparni=null;
		while(dvizi!=null) {
			int sum=0;
			if(dvizi.element%2==0) { //Paren
				sum=sum+dvizi.element;
				if(dvizi.succ!=null) {
					parni=dvizi.succ;
				}else {
					break;
				}
				while(parni!=null && parni.element%2 ==0) {
					sum=sum+parni.element;
					SLLNode<Integer> pom=parni;
				...
				while(neparni!=null && neparni.element%2 !=0) {
					sum=sum+neparni.element;
					SLLNode<Integer> pom=neparni;
