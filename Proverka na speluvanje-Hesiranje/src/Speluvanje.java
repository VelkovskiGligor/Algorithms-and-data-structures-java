	...
	public int hashCode() {
		...
		*/
		...
		for(int i=0;i<zbor.length();i++) {
			pom+= zbor.charAt(i)*31 ^(zbor.length()-1-i);
		}
		return pom;
	}
	@Override
...
public class Speluvanje {
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < N; i++) {
			...
			Zbor zbor=new Zbor(red);
			tabela.insert(zbor, red);
		}
		...
		for (int i = 0; i < pom.length; i++) {
			pom[i]=pom[i].toLowerCase();
			String zbor=pom[i];
			if(zbor.charAt(zbor.length()-1)=='.' 
					|| zbor.charAt(zbor.length()-1)==','
					|| zbor.charAt(zbor.length()-1)=='!'
					|| zbor.charAt(zbor.length()-1)=='?' ) {
				zbor=zbor.substring(0,zbor.length()-1);
			}
		
			int b= tabela.search(new Zbor(zbor));
			if(b==-1) {
				System.err.println(zbor);
				isOkej=false;
			}
