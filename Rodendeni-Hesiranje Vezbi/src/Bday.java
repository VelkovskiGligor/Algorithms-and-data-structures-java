...
public class Bday {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		CBHT<String,Datum> baza=new CBHT<String,Datum>(2*N);
		for (int i = 0; i <N; i++) {
			String red=input.readLine();
			String ime=red.split(" ")[0];
			String pom[]=red.split(" ");
			String datum= pom[1];
			String [] pom_dat=datum.split("\\.");
			String mesec=pom_dat[1];
			//System.err.println(mesec);
			SLLNode<MapEntry<String,Datum>> get=baza.search(mesec);
			if(get==null) {
				baza.insert(mesec, new Datum(ime));
			}else {
				String getime=get.element.value.iminja;
				if(!getime.contains(ime)) {
					getime=getime+" "+ime;
					baza.insert(mesec, new Datum(getime));
				}
				
			}
		}
		String br=input.readLine();
		SLLNode<MapEntry<String,Datum>> get=baza.search(br);
		if(get!=null) {
			System.err.println(get.element.value.iminja);
		}else {
